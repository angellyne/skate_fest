...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    if ($action === 'cadastrar_evento') {
        if (!isRepresentanteLogado()) {
            echo json_encode(['success' => false, 'errors' => ['Você precisa estar logado!']]);
            exit;
        }
        
        $erros = [];
        if (empty($_POST['nome_evento'])) $erros[] = 'Nome do evento é obrigatório';
        if (empty($_POST['data_evento'])) $erros[] = 'Data do evento é obrigatória';
        if (empty($_POST['cidade'])) $erros[] = 'Cidade é obrigatória';
        if (empty($_POST['estado'])) $erros[] = 'Estado é obrigatório';
        if (empty($_POST['local_evento'])) $erros[] = 'Local do evento é obrigatório';
        
        if (empty($erros)) {
            $stmt = $pdo->prepare("INSERT INTO eventos (nome_evento, data_evento, cidade, estado, local_evento, representante_id) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$_POST['nome_evento'], $_POST['data_evento'], $_POST['cidade'], $_POST['estado'], $_POST['local_evento'], $_SESSION['representante_id']]);
    ...
    if ($action === 'cadastrar_skater_competicao') {
        ...
        if (empty($erros)) {
            try {
                ...
                $id = $pdo->lastInsertId();
                if ($id > 0) {
                    echo json_encode(['success' => true, 'message' => 'Skatista cadastrado com sucesso!']);
                } else {
                    echo json_encode(['success' => false, 'errors' => ['Erro ao inserir no banco']]);
                }
            } catch(PDOException $e) {
                echo json_encode(['success' => false, 'errors' => ['Erro SQL: ' . $e->getMessage()]]);
            }
        } else {
            echo json_encode(['success' => false, 'errors' => $erros]);
        }
    ...
    if ($action === 'excluir_skater') {
        ...
        if ($id > 0) {
            $stmt = $pdo->prepare("DELETE FROM skatistas_competicao WHERE id = ?");
            $stmt->execute([$id]);
            echo json_encode(['success' => true]);
        } else {
    ...
    if ($action === 'resetar_competicao') {
        ...
        try {
            ...
            echo json_encode(['success' => true, 'message' => 'Competição resetada com sucesso!']);
        } catch(PDOException $e) {
            echo json_encode(['success' => false, 'errors' => ['Erro ao resetar: ' . $e->getMessage()]]);
        }
        exit;
    }
...
// Carregar dados para as páginas
$pagina = $_GET['pagina'] ?? 'home';

try {
    $eventos = $pdo->query("SELECT e.*, r.nome_representante FROM eventos e LEFT JOIN representantes r ON e.representante_id = r.id ORDER BY e.data_evento DESC")->fetchAll();
} catch(PDOException $e) {
    $eventos = [];
}
...
try {
    $stats_comp = $pdo->query("SELECT COUNT(*) as total, IFNULL(ROUND(AVG(media_geral), 1), 0) as media, IFNULL(ROUND(MAX(media_geral), 1), 0) as maior FROM skatistas_competicao")->fetch();
} catch(PDOException $e) {
    $stats_comp = ['total' => 0, 'media' => 0, 'maior' => 0];
}
...
<head>
    ...
    <title>SkateFest Brasil • Eventos e Competições</title>
    ...
    <style>
        * {
        ...
        .hero {
            text-align: center;
            padding: 60px 20px;
            background: linear-gradient(135deg, rgba(28, 11, 43, 0.8), rgba(48, 28, 65, 0.6));
            border-radius: 50px;
            margin-bottom: 50px;
        ...
        .hero::before {
            ...
            width: 200%;
            height: 200%;
            background: radial-gradient(circle, rgba(111,149,255,0.1) 0%, transparent 70%);
        ...
        .hero h1 {
            font-family: 'Barlow Condensed', sans-serif;
            font-size: 4rem;
            background: linear-gradient(45deg, var(--white), var(--light-blue), var(--vibrant-blue));
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            margin-bottom: 20px;
            position: relative;
        ...
        .card::before {
            ...
            width: 100%;
            height: 100%;
            background: linear-gradient(90deg, transparent, rgba(111,149,255,0.1), transparent);
        ...
        .card:hover {
            ...
            box-shadow: 0 15px 40px rgba(111,149,255,0.2);
        }
        ...
        .btn:hover {
            ...
            box-shadow: 0 8px 25px rgba(111, 149, 255, 0.4);
            letter-spacing: 1px;
        }
        ...
        .form-group select:focus {
            outline: none;
            border-color: var(--light-blue);
            box-shadow: 0 0 0 3px rgba(111, 149, 255, 0.2);
        }
        ...
        .card-header {
            background: linear-gradient(90deg, #413B6B, #5C65C0);
            padding: 20px 30px;
        }
        ...
        .stat-card {
            background: linear-gradient(135deg, #1C0B2B, #413B6B);
            padding: 15px;
            text-align: center;
            border-radius: 18px;
        }
        ...
        @keyframes floatGlow {
            0%, 100% { transform: translateX(-50%) translateY(0); filter: drop-shadow(0 0 8px gold); }
            50% { transform: translateX(-50%) translateY(-8px); filter: drop-shadow(0 0 20px gold); }
        }
        ...
        @media (max-width: 968px) {
            .grid-competicao { grid-template-columns: 1fr; }
...
<body>
    <nav class="navbar">
        <div class="navbar-container">
            ...
            <div class="nav-links">
                ...
                <?php if(isRepresentanteLogado()): ?>
                    <div class="user-info">
                        <span>👋 <?= $_SESSION['representante_nome'] ?></span>
    ...
    <main class="container">
        <?php if($pagina == 'home'): ?>
            <section class="hero">
                ...
                <div class="hero-stats">
                    <div class="hero-stat">
                        ...
                        <div class="hero-stat-label">Eventos Realizados</div>
                    </div>
                    <div class="hero-stat">
                        <div class="hero-stat-number">+5.000</div>
                        <div class="hero-stat-label">Skatistas Participantes</div>
                    </div>
                    <div class="hero-stat">
                        <div class="hero-stat-number">+20</div>
                        <div class="hero-stat-label">Cidades Atendidas</div>
                    </div>
                </div>
                <div class="hero-image">
                    <img src="img/aaaaaa.png" alt="Skate action">
                </div>
            </section>
            ...
            <div class="cards-grid">
                <div class="card">
                    ...
                    <h3>Competições</h3>
                    <p>Participe das melhores competições de skate do Brasil. Sistema de pontuação profissional e ranking atualizado.</p>
                    <?php if(isRepresentanteLogado()): ?>
                        <a href="?pagina=competicao" class="btn">Ver Competições →</a>
                    <?php else: ?>
                        <button class="btn" onclick="abrirModalLogin()">Faça login para acessar →</button>
                    <?php endif; ?>
                </div>
                <div class="card">
                    <div class="card-icon">📅</div>
                    <h3>Eventos</h3>
                    <p>Confira a agenda de eventos de skate em todo o Brasil. Encontre um perto de você e participe!</p>
                    <a href="?pagina=eventos" class="btn">Ver Eventos →</a>
                </div>
                <div class="card">
                    <div class="card-icon">💡</div>
                    <h3>Dicas</h3>
                    <p>Aprenda novas manobras, cuide do seu equipamento e evolua seu estilo com nossos especialistas.</p>
                    <a href="?pagina=dicas" class="btn">Ver Dicas →</a>
                </div>
                <div class="card">
                    <div class="card-icon">🛒</div>
                    <h3>Produtos</h3>
                    <p>As melhores marcas de skate com preços exclusivos para a comunidade SkateFest.</p>
                    <a href="?pagina=produtos" class="btn">Comprar →</a>
                </div>
                <div class="card">
                    <div class="card-icon">👔</div>
                    <h3>Sou Representante</h3>
                    <p>Cadastre seu evento e alcance skatistas de todo o Brasil. Área exclusiva para organizadores.</p>
                    <button class="btn" onclick="abrirModalLogin()">Área Restrita →</button>
                </div>
                <div class="card">
                    <div class="card-icon">🛹</div>
                    <h3>Sou Skatista</h3>
                    <p>Cadastre-se nos eventos, mostre seu talento nas competições e faça parte da maior comunidade de skate do Brasil.</p>
                    <a href="?pagina=cadastro-skatista" class="btn">Me Inscrever →</a>
                </div>
            </div>
        ...
        <?php elseif($pagina == 'eventos'): ?>
            <section class="hero">
                ...
                <p>Confira os próximos eventos em todo o Brasil</p>
            </section>
            ...
            <div style="display: grid; gap: 20px;">
                <?php if(empty($eventos)): ?>
                    <div class="card" style="text-align: center;">
                        ...
                        <?php if(isRepresentanteLogado()): ?>
                            <button class="btn" style="margin-top: 20px;" onclick="abrirModalEvento()">Cadastrar Evento</button>
                        <?php else: ?>
                            <button class="btn" style="margin-top: 20px;" onclick="abrirModalLogin()">Login para cadastrar evento</button>
                        <?php endif; ?>
                    </div>
                <?php else: ?>
                    <?php foreach($eventos as $evento): ?>
                        <div class="evento-card">
                            <div class="evento-info">
                                ...
                                <p>📍 <?= htmlspecialchars($evento['local_evento']) ?> - <?= htmlspecialchars($evento['cidade'])?>/<?= htmlspecialchars($evento['estado']) ?></p>
                                <p>👤 Organizador: <?= htmlspecialchars($evento['nome_representante'] ?? 'Desconhecido') ?></p>
                            </div>
                            <div class="evento-data" style="text-align: center;">
                                <div style="font-size: 2rem; font-weight: 900; color: white;"><?= date('d', strtotime($evento['data_evento'])) ?></div>
                                <div style="color: var(--light-blue);"><?= strtoupper(date('M', strtotime($evento['data_evento']))) ?></div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
        ...
        <?php elseif($pagina == 'competicao'): ?>
                ...
                <div class="grid-competicao">
                    <div class="card-competicao">
                        ...
                        <form id="formCompeticao" class="form-competicao">
                            <div class="input-group">
                                ...
                                <input type="text" name="nome" id="comp_nome" placeholder="Ex: TONY HAWK" required>
                            </div>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                                <div class="input-group">
                                    <label>PAÍS</label>
                                    <input type="text" name="pais" id="comp_pais" placeholder="BRASIL" required>
                                </div>
                                <div class="input-group">
                                    <label>IDADE</label>
                                    <input type="number" name="idade" id="comp_idade" placeholder="18" min="10" max="60" required>
                                </div>
                            </div>
                    ...
                    <div class="card-competicao">
                        ...
                        <div class="stats-grid">
                            <div class="stat-card">
                                <div class="stat-valor" id="totalStats"><?= $stats_comp['total'] ?></div>
                                <div style="color:white; font-size:0.8rem;">PARTICIPANTES</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-valor" id="mediaStats"><?= $stats_comp['media'] ?></div>
                                <div style="color:white; font-size:0.8rem;">MÉDIA GERAL</div>
                            </div>
                            <div class="stat-card">
                                <div class="stat-valor" id="maiorStats"><?= $stats_comp['maior'] ?></div>
                                <div style="color:white; font-size:0.8rem;">MAIOR NOTA</div>
                            </div>
                        </div>
                        <div class="podium">
                            <div class="podium-item">
                                ...
                                <div class="podium-info" id="podium2">
                                    <?php if(isset($top3[1])): ?>
                                        <strong><?= htmlspecialchars($top3[1]['nome']) ?></strong><br>Média: <?= $top3[1]['media_geral'] ?>
                                    <?php else: ?>
                                        Aguardando...
                                    <?php endif; ?>
                                </div>
                            ...
                            <div class="podium-item">
                                ...
                                <div class="podium-info" id="podium1">
                                    <?php if(isset($top3[0])): ?>
                                        <strong><?= htmlspecialchars($top3[0]['nome']) ?></strong><br>Média: <?= $top3[0]['media_geral'] ?>
                                    <?php else: ?>
                                        Aguardando...
                                    <?php endif; ?>
                                </div>
                            ...
                            <div class="podium-item">
                                ...
                                <div class="podium-info" id="podium3">
                                    <?php if(isset($top3[2])): ?>
                                        <strong><?= htmlspecialchars($top3[2]['nome']) ?></strong><br>Média: <?= $top3[2]['media_geral'] ?>
                                    <?php else: ?>
                                        Aguardando...
                                    <?php endif; ?>
                                </div>
                                ...
                                <?php foreach($skaters_comp as $i => $s): ?>
                                    <div class="skater-item">
                                        ...
                                        <div style="flex:1; margin-left:15px;">
                                            <div style="color:white; font-weight:bold;"><?= htmlspecialchars($s['nome']) ?></div>
                                            <div style="color:#6F95FF; font-size:0.8rem;"><?= htmlspecialchars($s['pais']) ?> • <?= $s['idade'] ?> anos</div>
                                        </div>
                                        <span style="font-family:'Barlow Condensed'; font-size:1.6rem; font-weight:900; color:#6F95FF;"><?= $s['media_geral'] ?></span>
                                    </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </div>
            ...
            <?php else: ?>
                <div class="acesso-negado">
                    ...
                    <p>Esta área é exclusiva para representantes de eventos.</p>
                    <button class="btn" onclick="abrirModalLogin()">FAZER LOGIN</button>
                </div>
            <?php endif; ?>
        ...
        <?php elseif($pagina == 'dicas'): ?>
            <section class="hero">
                ...
                <p>Aprenda com os melhores e evolua seu estilo</p>
            </section>
            <div class="dicas-grid">
                <?php foreach($dicas as $dica): ?>
                    <div class="dica-card">
                        <span class="dica-categoria"><?= htmlspecialchars($dica['categoria']) ?></span>
                        <h3><?= htmlspecialchars($dica['titulo']) ?></h3>
                        <p><?= htmlspecialchars($dica['conteudo']) ?></p>
                    </div>
                <?php endforeach; ?>
            </div>
        ...
        <?php elseif($pagina == 'produtos'): ?>
            <section class="hero">
                <h1>🛒 LOJA PARCEIRA</h1>
                <p>As melhores marcas com preços especiais</p>
            </section>
        ...
        <?php elseif($pagina == 'cadastro-skatista'): ?>
            <div class="form-container">
                <h2>🛹 INSCRIÇÃO SKATISTA</h2>
                <form id="formSkatista">
                    <div class="form-group"><label>NOME COMPLETO</label><input type="text" name="nome_skatista" placeholder="Seu nome" required></div>
                    <div class="form-group"><label>EMAIL</label><input type="email" name="email" placeholder="seuemail@exemplo.com" required></div>
                    <div class="form-group"><label>TELEFONE</label><input type="tel" name="telefone" placeholder="(11) 99999-9999" required></div>
                    <div class="form-group"><label>IDADE</label><input type="number" name="idade" min="5" max="70" placeholder="18" required></div>
    ...
    <div id="modalLogin" class="modal">
        <div class="modal-content">
            ...
            <p style="color: var(--light-blue); margin-top: 20px; text-align: center; font-size: 0.8rem;">Demo: admin@skatefest.com / admin123</p>
    ...
    <div id="modalEvento" class="modal">
        <div class="modal-content" style="max-width: 500px;">
            <span class="close-modal" onclick="fecharModalEvento()">&times;</span>
            ...
            <form id="formEvento">
                ...
                <button type="submit" class="btn" style="width:100%;">CADASTRAR EVENTO</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        window.onclick = function(event) {
            ...
            if (event.target == modalEvento) modalEvento.classList.remove('active');
        }
        ...
        if(formEvento) {
            formEvento.addEventListener('submit', async (e) => {
                ...
                formData.append('action', 'cadastrar_evento');
                const response = await fetch('', { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest' }, body: formData });
                const data = await response.json();
                if(data.success) {
                    mostrarToast('✅ Evento cadastrado com sucesso!', 'success');
                    formEvento.reset();
                    fecharModalEvento();
            ...
            if(resetBtn) {
                resetBtn.addEventListener('click', async () => {
                    if(confirm('🔥 TEM CERTEZA? TODOS OS SKATISTAS SERÃO REMOVIDOS! 🔥')) {
                        ...
                        formDataReset.append('action', 'resetar_competicao');
                        const response = await fetch('', { method: 'POST', headers: { 'X-Requested-With': 'XMLHttpRequest' }, body: formDataReset });
                        const data = await response.json();
                        if(data.success) {
                            mostrarToast('🏆 Competição resetada com sucesso!', 'success');
                            await atualizarCompeticao();
                        } else {
                            mostrarToast('❌ ' + (data.errors ? data.errors.join(', ') : 'Erro ao resetar competição!'), 'error');
                        }
                    }
                });
            }
